    ...
    <nav className="sticky top-0 z-50 w-full bg-canvas/85 backdrop-blur-md border-b border-ink/5">
      <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link to={signedIn ? "/account" : "/auth"} aria-label="Account" className="text-sm font-medium hover:text-gold transition-colors" title={signedIn ? "Account" : "Sign in"}>
            <User className="size-4" />
          </Link>
          <Link to="/wishlist" aria-label="Wishlist" className="relative text-sm font-medium hover:text-gold transition-colors">
          ...
          <Link to="/cart" className="text-xs font-medium tracking-widest flex items-center gap-1.5 hover:text-gold transition-colors">
            <ShoppingBag className="size-4" />
            <span className="hidden sm:inline">CART ({cartCount})</span>
            {cartCount > 0 && <span className="sm:hidden bg-gold text-ink rounded-full size-4 grid place-items-center text-[9px] font-semibold">{cartCount}</span>}
          </Link>
          <button
            className="md:hidden"
            onClick={() => setOpen((o) => !o)}
      ...
      {open && (
        <div className="md:hidden border-t border-ink/5 bg-canvas">
          <div className="px-4 py-4 flex flex-col gap-3">
            ...
            <Link to="/faq" onClick={() => setOpen(false)} className="text-sm font-medium tracking-widest">
              FAQ
            </Link>
            <Link to="/auth" onClick={() => setOpen(false)} className="text-sm font-medium tracking-widest text-gold">
