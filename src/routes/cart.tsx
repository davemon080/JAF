...
function CartPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-6 py-12">
      <h1 className="font-display text-5xl md:text-7xl font-semibold tracking-tighter mb-10">CART.</h1>
      ...
      {items.length === 0 ? (
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-12">
          <ul className="space-y-6">
            {items.map((item) => (
              <li key={item.idx} className="flex gap-4 border-b border-ink/10 pb-6">
                <Link to="/product/$slug" params={{ slug: item.product.slug }} className="w-24 sm:w-32 aspect-[4/5] bg-zinc-100 shrink-0">
                ...
                <div className="flex-1 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  <div>
                    <Link to="/product/$slug" params={{ slug: item.product.slug }} className="text-sm font-semibold tracking-widest uppercase">
                      {item.product.name}
                    </Link>
                    ...
                    <div className="inline-flex items-center border border-ink/20 mt-3">
                      <button onClick={() => setQty(item.idx, item.qty - 1)} className="size-8 grid place-items-center">
                  ...
                  <div className="flex flex-col items-end gap-3">
                    <p className="text-sm font-medium">{formatNaira(item.product.price * item.qty)}</p>
                    <button onClick={() => remove(item.idx)} aria-label="Remove" className="text-ink-soft hover:text-ink">
                      <X className="size-4" />
                    </button>
                  </div>
                </div>
          ...
          <aside className="lg:sticky lg:top-20 lg:self-start bg-ink text-canvas p-6 space-y-5">
            <h2 className="text-xs tracking-widest uppercase border-b border-canvas/15 pb-3">Summary</h2>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-canvas/60">Subtotal</span><span>{formatNaira(subtotal)}</span></div>
              <div className="flex justify-between"><span className="text-canvas/60">Delivery</span><span>at checkout</span></div>
            </div>
            ...
            <Link to="/checkout" className="block text-center bg-canvas text-ink text-xs font-medium tracking-widest uppercase py-3 hover:bg-canvas/90">
              Checkout
            </Link>
            <Link to="/shop" className="block text-center text-[10px] tracking-widest uppercase text-canvas/60 hover:text-canvas">
              Keep shopping
            </Link>
          </aside>
        </div>
